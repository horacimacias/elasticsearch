...
[[email-account-attributes]]
  ...
  `smtp.ssl.trust`;;
  Hostname for which certificates will be trusted, even if they are self-signed or signed by an unknown CA.
  This is passed to javamail properties; see https://javaee.github.io/javamail/docs/api/com/sun/mail/smtp/package-summary.html for more information. This is an alternative to updating key/trust stores accordingly. 
  *Note:* Make sure you understand the security implications of using such setting.
